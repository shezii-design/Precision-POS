...
export const PurchaseOrderViewModal: React.FC<PurchaseOrderViewModalProps> = ({
  ...
  const getStatusBadge = () => {
    switch (po.status) {
      case 'completed':
        return (
          <span className="px-2.5 py-1 bg-emerald-100 text-emerald-800 text-xs font-black rounded-lg border border-emerald-300/80 flex items-center gap-1.5 shadow-sm">
      ...
      case 'pending_bill':
        return (
          <span className="px-2.5 py-1 bg-amber-100 text-amber-900 text-xs font-black rounded-lg border border-amber-300/80 flex items-center gap-1.5 shadow-sm">
      ...
      case 'ordered':
        return (
          <span className="px-2.5 py-1 bg-blue-100 text-blue-800 text-xs font-black rounded-lg border border-blue-300/80 flex items-center gap-1.5 shadow-sm">
      ...
      default:
        return (
          <span className="px-2.5 py-1 bg-slate-200 text-slate-700 text-xs font-black rounded-lg border border-slate-300/80 flex items-center gap-1.5 shadow-sm">
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-2 sm:p-4 bg-slate-950/75 backdrop-blur-xs animate-in fade-in duration-200">
      ...
      >
        {/* Header Actions Bar */}
        <div className="bg-slate-900 px-5 sm:px-7 py-4 text-white flex items-center justify-between shrink-0 shadow-sm print:hidden">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-white/10 backdrop-blur-md flex items-center justify-center font-bold text-amber-300 border border-white/10 shadow-sm">
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={handlePrint}
              className="px-3 py-1.5 bg-white/15 hover:bg-white/25 active:bg-white/30 text-white text-xs font-bold rounded-xl border border-white/20 transition-colors flex items-center gap-1.5 cursor-pointer shadow-sm"
            >
            ...
            {(po.status === 'pending_bill' || po.status === 'ordered') && (onReceive || onOpenReceiveModal) && (
              <button
                ...
                className={`px-3.5 py-1.5 text-xs font-black rounded-xl transition-colors flex items-center gap-1.5 cursor-pointer shadow-sm ${
        ...
        <div ref={printAreaRef} className="flex-1 overflow-y-auto p-5 sm:p-8 space-y-6 bg-white print:p-0">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs bg-slate-200 p-4 rounded-2xl border border-slate-200">
            <div>
          ...
          <div className="border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
            <table className="w-full min-w-[700px] text-left border-collapse text-xs">
              <thead>
                <tr className="bg-slate-200 text-slate-800 font-bold border-b border-slate-200 text-[11px]">
              ...
              <tbody className="divide-y divide-slate-100">
                {po.items.map((item, idx) => {
                  ...
                  return (
                    <tr key={item.id || idx} className="hover:bg-slate-300/80">
                      ...
                      <td className="py-2.5 px-2 text-center">
                        <span className={`px-2 py-0.5 rounded-md font-black text-xs ${
                          po.isStockReceived ? 'bg-emerald-100 text-emerald-900' : 'bg-slate-200 text-slate-600'
                        }`}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 items-start">
            ...
            <div className="space-y-3">
              {po.notes && (
                <div className="bg-slate-200 p-3.5 rounded-2xl border border-slate-200 text-xs">
            ...
            <div className="bg-slate-200 p-4 rounded-2xl border border-slate-200 space-y-2.5 text-xs">
        ...
        {/* Modal Footer */}
        <div className="bg-slate-200 border-t border-slate-200 px-5 py-3.5 flex items-center justify-between gap-3 shrink-0 print:hidden">
          <button
            type="button"
            onClick={onClose}
