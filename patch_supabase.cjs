...
const regex = /const chunkSize = 100;\s*for\s*\([^\{]+\{\s*const chunk = ([a-zA-Z]+)\.slice[^\{]+\{\s*const\s*\{\s*error\s*\}\s*=\s*await client\s*\.from\('([^']+)'\)\s*\.upsert\([^,]+,\s*\{\s*onConflict:\s*'([^']+)'\s*\}\);\s*if\s*\(error\)\s*\{\s*return\s*\{\s*success:\s*false,\s*count:\s*i,\s*error:\s*error\.message\s*\};\s*\}\s*\}\s*return\s*\{\s*success:\s*true,\s*count:\s*[a-zA-Z]+\.length\s*\};/g;

code = code.replace(regex, (match, arrName, tableName, idCol) => {
  return `return await exactSyncRows(client, '${tableName}', ${arrName}, '${idCol}');`;
});
